...
while(pastel_idx == 0 && dist >= dist_min)
    while(i < length(ranges) - 1 && pastel_count < pastel_n)
        ...
        x_d = norm([lidar.sensorOffset(1), lidar.sensorOffset(1)]) + ranges(j)*cos(lidar.scanAngles(j));
        y_d = ranges(j)*sin(lidar.scanAngles(j));
...
free_angle = lidar.scanAngles(free_scan);
